...
export default function WokThinking() {
  ...
  const advice = () => {
    ...
    // MICROWAVE analysis
    if(pMW===0) a.push({t:"ok",param:"📡 Microwave 0%",m:"Pure impingement cook. Zero MW means zero molecular excitation — all heating is from hot air contact and conduction. This is correct for: crackling (moisture at surface would destroy the skin), pastry (steam from MW would make pastry soggy), artisan bread (crust requires dry heat only). Settings are identical across all oven models at 0% MW."});
    ...
    else if(pFan===100) a.push({t:pTime>60?"warn":"ok",param:"💨 Fan 100%",m:pTime>60?`Maximum impingement for ${pTime} seconds — at full fan for this duration, surface colour will be very aggressive. Reduce fan by 10–15% if browning too fast, or reduce temperature by 10–15°C. 100% fan for extended periods is correct for pizza, naan and crackling only.`:"Maximum impingement at 100% for a short burst. Correct for products needing rapid surface crust development — toast, naan, thin pastry. Zero-MW at 100% fan replicates a tandoor or deck oven environment."});
            ...
            <div style={{margin:"16px 0",display:"grid",gridTemplateColumns:"1fr 1fr",gap:"10px"}}>
              {[
                ...
                {ctrl:"Microwave Assist",ic:"📡",range:"0–100%",col:"#5090c0",
                 ...
                 decrease:"More reliance on conduction from the surface inward. At 0% MW, the oven is a pure hot-air impingement convection oven. Correct for anything requiring a completely dry surface: crackling (surface moisture destroys the skin), pastry (MW steam makes pastry soggy), artisan bread.",
